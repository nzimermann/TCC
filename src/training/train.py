"""Train the YOLO11n plate detector.
...
--smoke-test on the command line — either works):
  ...
  - SMOKE_TEST = True: fast pipeline check. Builds a tiny subset
    ...
    --smoke-epochs to get a realistic per-epoch timing reading before
...
def build_smoke_subset(n_images: int) -> Path:
    ...
    into a tiny throwaway dataset, used for both train and val. Returns the
    path to the small data.yaml describing it."""
...
def main():
    ...
    if args.resume:
        ...
        # run (Ultralytics' own bundled coco8 example, 100 epochs) instead
        # of resuming anything. Caught this exact failure mode by hand
        # before shipping it, so it's worth a hard stop here instead.
